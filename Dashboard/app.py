...
async def get_csv_dataset(name: str):
    ...
    try:
        ...
        return {"dataset": name, "columns": columns, "data": records}
    except Exception as e:
    ...
    try:
        ...
        if has_changes:
            ...
            commit_proc = await asyncio.create_subprocess_exec(
                ...
                "commit",
                "-m",
                "Auto-trigger OTA: live deployment update",
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
            )
        else:
            ...
            commit_proc = await asyncio.create_subprocess_exec(
                ...
                "--allow-empty",
                "-m",
                "Auto-trigger OTA: matrix compile [CI Skip]",
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
            )
        ...
        hash_proc = await asyncio.create_subprocess_exec("git", "rev-parse", "HEAD", stdout=asyncio.subprocess.PIPE)
...
async def get_ota_workflow_status(sha: str = None):
    try:
        ...
        workflow_url = f"{Config.BASE_URL}{Envs.OWNER}/{Envs.REPO}/actions/workflows/{Envs.WORKFLOW_ID}/runs"

        response = requests.get(workflow_url, headers=Config.GITHUB_HEADERS, params={"per_page": 10})
