...
from NHIOTSub.models.payloads import HeartbeatPayload, OTAStatusPayload
...
class NHIOTSubscriber:
    ...
    def run_unit_tests(self, binary_path: str, target: str) -> bool:
        ...
        for fn, args, expected in test_cases:
            ...
            if stderr or expected not in out_clean:
                self.logger.warning(
                    f"  [FAIL] Unit Test {fn}({args}) failed! Output: '{stdout.strip()}', Error: '{stderr.strip()}' (Expected: '{expected}')"
                )
                return False
            passed_count += 1
            self.logger.info(
    ...
    def revert_to_previous_github_build(
        ...
        for prev_run in recent_runs:
            ...
            self.logger.info(
                f"Attempting GitHub Version Revert -> Build Run #{prev_run.id} (SHA: {prev_sha[:7] if prev_sha else 'unknown'})..."
            )
        ...
        if artifact:
            ...
            try:
                ...
                if self.run_unit_tests(downloaded_path, target):
                    ...
                    self.send_ota_notification(
                        "SUCCESS",
                        f"All unit tests passed for build #{run.id}. Binary operational.",
                        commit_sha,
                    )
                    self.logger.info(
                        f"SUCCESS: Subscriber loaded operational artifact '{target}' for branch '{branch}' -> {self.current_file_path}"
                    )
                    self.logger.info(
                        f"Subscriber active with run #{run.id} on branch '{branch}'. Monitoring GitHub for next build..."
                    )
                    return self.current_file_path
                else:
                    ...
                    if reverted_path:
                        ...
                        self.send_ota_notification(
                            "FAILURE",
                            f"Unit tests failed on run #{run.id} and no historical build could be restored.",
                            commit_sha,
                        )
                        return self.current_file_path
                ...
                self.logger.warning(
                    f"Download/Verification failed: {download_error}. Triggering GitHub version revert..."
                )
                reverted_path = self.revert_to_previous_github_build(run.id, branch)
                if reverted_path:
                    return reverted_path
                    ...
                    self.send_ota_notification(
                        "FAILURE",
                        f"Artifact download/verification failed: {download_error}",
                        commit_sha,
                    )
                    return self.current_file_path
        else:
            ...
            self.logger.warning(
                f"Build run #{run.id} failed or missing artifact. Reverting to previous successful GitHub build history..."
            )
            reverted_path = self.revert_to_previous_github_build(run.id, branch)
            if reverted_path:
                return reverted_path
    ...
    def _on_branch_changed(self, new_branch: str) -> str | None:
        """Callback invoked when publisher sends a branch change payload over MQTT."""
        self.logger.info(
            f"Subscriber notified of branch change to '{new_branch}' — resetting run tracker & pulling artifact."
        )
        self.last_processed_run_id = None
        self.branch_changed = False
    ...
    def monitor_workflow(self) -> None:
        ...
        if not self.current_file_path:
            self.logger.info(
                f"No local binary found on startup. Attempting initial artifact pull for branch '{Envs.BRANCH}'..."
            )
        ...
        while True:
            ...
            self.logger.info(
                f"Polling GitHub Actions API for branch '{Envs.BRANCH}'... (local HEAD: {local_sha[:7] if local_sha else 'unknown'})"
            )
            ...
            if self.last_processed_run_id != run.id:
                if run.status == "completed":
                    self.logger.info(
                        f"New completed build run #{run.id} (SHA: {sha_str}) detected on branch '{Envs.BRANCH}' — fetching artifact..."
                    )
                    self.fetch_artifact_for_branch(Envs.BRANCH)
                elif run.status in ("in_progress", "queued", "requested", "waiting"):
                    self.logger.info(
                        f"New build run #{run.id} (SHA: {sha_str}) on branch '{Envs.BRANCH}' is '{run.status}' — waiting for build to complete..."
                    )
            else:
                self.logger.info(
                    f"Waiting for next GitHub build on branch '{Envs.BRANCH}'... (Active: run #{run.id} [{sha_str}] | Target: '{target}' | Next check in {Envs.POLL_INTERVAL}s)"
                )
